...
JirashApi.prototype.createIssue = function createIssue(opts, cb) {
    ...
    assert.string(fields.project.id || fields.project.key,
        'fields.project.id || fields.project.key');
    assert.object(fields.issuetype, 'fields.issuetype');
    assert.string(fields.issuetype.id || fields.issuetype.name,
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function createIt(ctx, next) {
            ...
            }, ctx.createOpts, function (err, req, res, body) {
                if (err) {
                    next(err);
                } else {
                    ctx.issue = body;
        ...
        function getIt(ctx, next) {
            ctx.jiraClient.get({
                ...
                query: _stripNullOrUndefined({
                    ...
                    expand: opts.expand ? 'projects.issuetypes.fields' : null
                })
            }, function (err, req, res, body) {
                if (err) {
                    next(err);
...
JirashApi.prototype.createVersion = function createVersion(opts, cb) {
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function createIt(ctx, next) {
            ...
            }, opts, function (err, req, res, body) {
                if (err) {
                    next(err);
                } else {
                    ctx.ver = body;

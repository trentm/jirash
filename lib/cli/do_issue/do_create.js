...
function fieldsFromEditor(ctx, next) {
    ...
    if (ctx.formFile) {
        assert.string(ctx.fileText, 'ctx.fileText');
        text = ctx.fileText;
    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                ...
                function readFile(ctx, next) {
                    ...
                    if (opts.f === '-') {
                        common.readStdin(function gotStdin(text) {
                            ctx.fileText = text;
                            next();
                        });
                    } else {
                        fs.readFile(opts.f, 'utf8', function (readErr, text) {
                            ctx.fileText = text;
                            next(readErr);
                        });
                    }
                ...
                function fieldsFromFile(ctx, next) {
                    ...
                    var fileFields;
                    try {
                        fileFields = JSON.parse(ctx.fileText);
                    } catch (_jsonParseErr) {
                        fileFields = null;
                    }
                    if (fileFields === null) {
...
do_create.options = [
    ...
    },
    {
        ...
        help:
            ...
            'used for re-editing after an earlier creation error). Use "-" ' +
            'to read from stdin.'
    },
    {

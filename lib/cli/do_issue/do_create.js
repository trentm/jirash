...
function parseIssueForm(text) {
    ...
    while (i < lines.length) {
        ...
        if (idx === -1) {
            throw new VError(
                ...
                'line %d is not in "Field: value" format: "%s"', i, line);
        }
    ...
    while (i < lines.length) {
        ...
        if (parsedFields.description.length !== 0
            || line.trim().length !== 0) {
            parsedFields.description.push(line);
        }
    }
    parsedFields.description
        = parsedFields.description.join('\n').trimRight();
...
function do_create(subcmd, opts, args, cb) {
    ...
    if (opts.f && !context.useEditor) {
        cb(new UsageError('"--no-edit" cannot be used with "-f FILE"'));
        return;
    }
    ...
    vasync.pipeline({arg: context, funcs: [
        /*
         * TODO
         * Get create metadata to support filling out and validating issue
         * form fields. We cache this for a long while because this takes
         * too long to run for each `jirash create ...` invocation.
         */
        //function getCreateMeta(ctx, next) {
        //    // TODO Consider caching with 'expand=true' which has other fields
        //    //      to consider. That's a big schema to work through however.
        //    //      E.g. could know if 'component' is required.
        //    ctx.cli.jirashApi.getCreateIssueMeta({
        //        projectKeys: [ctx.projectKey]
        //    }, function (err, meta) {
        //        ctx.meta = meta;
        //        console.log('meta: %s', JSON.stringify(meta, null, 4));
        //        next(err);
        //    })
        //},
        ...
        function fieldsFromInputs(ctx, next) {
            ...
            if (opts.components) {
                ctx.fields.components = opts.components
             ...
             * (issuetype.subtask=false) in the create metadata (ctx.meta).
             *
             * If given, do a match against all subtask=false names from ^^.
             */
            // TODO: issuetype, opts.type
        ...
        function fieldsFromEditor(ctx, next) {
            ...
            if (opts.f) {
                try {
                    text = fs.readFileSync(opts.f, 'utf8');
                } catch (readErr) {
                    next(readErr);
                    return;
                }

                ctx.editFilename = opts.f;
            } else {
                text = [
                    ...
                    'Type: ' + (ctx.fields.issuetype && ctx.fields.issuetype.name || ''),
                    'Assignee: ',
                    'Components: ',
                ...
                ctx.editFilename = format('./jirash-%s-%s.issue',
                    timestamp, ctx.projectKey);
            }
            ...
            var editAttempt = function () {
                common.editInEditor({
                    ...
                    noUnlink: true,
                    log: log
                }, function (err, updatedText, changed) {
                    if (err) {
                        next(err);
                        return;
                    }
                    ...
                    log.trace({parsedForm: parsedForm}, 'parsed issue form');
                    ...
                    Object.keys(parsedForm).forEach(function (field) {
                        switch (field) {
                        case 'summary':
                        ...
                        case 'type':
                            ...
                            ctx.fields.issuetype = {name: parsedForm.type};
                            break;
                        case 'assignee':
        ...
        function createIt(ctx, next) {
            log.trace({dryRun: opts.dry_run, fields: ctx.fields},
                'createIssue');

            if (opts.dry_run) {
                console.log('Creating (dry-run) %s issue:\n%s', ctx.projectKey,
        ...
        function printIt(ctx, next) {
            if (opts.json) {
                ...
                console.log('Created issue %s (%s/browse/%s).', ctx.issue.key,
                    ctx.jiraUrl, ctx.issue.key);
            }
            next();
        }
    ]}, function finish(err) {
        if (err) {
            if (context.editFilename) {
                console.log('Issue form was saved to "%s".\n'
                    + 'Use "jirash create -f %s %s" to retry.',
                    context.editFilename, context.editFilename,
                    context.projectKey);
            }
            cb(err);
             ...
             */
            log.trace({editFilename: context.editFilename},
                'unlink editFilename');
            fs.unlink(context.editFilename, function (unlinkErr) {
                if (unlinkErr) {
                    log.debug(unlinkErr, 'error unlinking editFilename');
                    console.error(
                        'warning: could not delete temporary file "%s": %s',
                        context.editFilename, unlinkErr);
                }
                cb();
            });
        } else {
...
do_create.options = [
    ...
    },
    {
        names: ['edit', 'e'],
        type: 'bool',
        help: 'Edit issue fields in your $EDITOR. This is the default '
            + 'behaviour.'
    },
    {
        names: ['no-edit', 'E'],
    ...
    },
    {
        ...
        type: 'string',
        helpArg: 'FILE',
        help: 'Path to a jirah issue form. Used for re-editing after a '
    ...
    },
    {
        ...
        type: 'string',
        helpArg: 'USER',
        help: 'User to which to assign the ticket. Use "me" to assign to '

    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                ...
                function listEm(ctx, next) {
                    ...
                    var searchStream = ctx.cli.jirashApi.createSearchStream({
                        jql: ctx.filter.jql,
                        fields: fields
                    });
                    ...
                    ctx.issues = [];
                    searchStream.on('readable', function searchReadable() {
                        var issue;
                        while ((issue = searchStream.read()) !== null) {
                            ctx.issues.push(issue);
                        }
                    });
                    searchStream.on('end', function doneSearch() {
                        next();
                    });
                },
                ...
                function printEm(ctx, next) {
                    if (opts.json) {
                        ...
                        ctx.issues.forEach(function anIssue(issue) {
                            ...
                            issue.components =
                                fields.components
                                    .map(function nameFromComp(comp) {
                                        return comp.name;
                                    })
                                    .join(',') || null;
                            issue.assignee =
                                ...
                                null;
                            issue.reporter =
                                (fields.reporter && fields.reporter.name)
                                || null;
                            issue.p =
                                (fields.priority && fields.priority.name[0])
                                || null;
                            issue.priority =

...
var columnsDefault = 'key,shortSummary,assignee,reporter,p,stat,created,updated'.split(/,/g);
var columnsDefaultLong = 'key,priority,status,type,reporter,assignee,created,updated,resolved,summary'.split(/,/g);
var columnsDefaultShort = 'key,summary'.split(/,/g);
        ...
        function getFilterByName(ctx, next) {
            ...
            ctx.cli.jirashApi.getFavouriteFilters(function (err, filters) {
                ...
                if (matches.length === 1) {
                    ...
                    errmsgs.push(format(
                        'filter term /\\b%s\\b/i is ambiguous, it '
                            + 'matches %d filters: "%s"',
                        filterIdOrName,
                        matches.length,
                        matches.map(
        ...
        function printEm(ctx, next) {
            if (opts.json) {
                ...
                ctx.issues.forEach(function (issue) {
                    ...
                    issue.shortSummary = (fields.summary.length > 40
    ...
    [
        {
            names: ['short', 'S'],
            type: 'bool',
            help: '"Short" output. Fewer column are displayed to show '
                + 'full summary.'
        }
    ]
);
